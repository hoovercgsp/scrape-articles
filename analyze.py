...
def analyze_word_cooccurrences(contents, window_size=5):
    ...
    for pair, count in sorted_cooccurrences[:10]:
        print(f"{pair}: {count}")
            ...
            if file_name.endswith('.txt'):  # Process only .txt files
                ...
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    if lines and lines[0].startswith("Title: "):
                        ...
                        print(f"Warning: File {file_name} does not have a valid title line.")
...
if __name__ == "__main__":
    ...
    folder_path = os.path.join(os.path.dirname(__file__), "ECNS", "politics_in_2024")
    ...
    for topic_id, doc_indices in topic_docs.items():
        ...
        for doc_index in doc_indices:
            print(f"  Document {doc_index}: {list(txt_files_content.keys())[doc_index]}")
        print()
